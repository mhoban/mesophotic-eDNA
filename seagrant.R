...
sample_data <- sample_data %>%
  mutate(
    comparison = case_when(
      station_grouping %in% c("China Walls","Hanauma Bay") ~ "hanauma",
      station_grouping %in% c("HIMB","Kaneohe North") ~ "kaneohe bay",
      station_grouping %in% c("Waialea MLCD","Waialea ref") ~ "waialea",
      station_grouping %in% c("Waikiki MLCD","Waikiki MLCD ref") ~ "waikiki",
      station_grouping %in% c("Kealakekua MLCD","Kealakekua ref") ~ "kealakekua",
      station_grouping %in% c("Old Kona Airport MLCD","Old Kona Airport ref") ~ "kona",
      TRUE ~ "none"
    ),
    comparison = factor(comparison,levels=c("hanauma","kaneohe bay","waikiki","kealakekua","waialea","kona")),
    sample_id = id
  )
...
ord_plotz <- map2(comm_ps,names(comm_ps),~{
  ...
  levs <- length(unique(sample_data(.x)$station_grouping))
  ...
  ord_plot <- plot_ordination(.x,ord,type="samples",color="station_grouping",shape="protection_status") +
    # facet_wrap(~comparison) +
    geom_point(size=8,aes(text=str_c(sample_id,"<br>",station))) + 
    ggtitle(str_glue("PCoA ordination: {descriptions[.y]}")) + 
    # scale_color_manual(values=rev(beyonce_palette(75))[5:12], name="Depth Zone") + 
    scale_color_manual(values=clr, name="Site") +
...
walk2(ord_plotz,names(ord_plotz),~{
  dir <- "~/projects/dissertation/admin/committee meetings/february 24, 2022/img/seagrant"
...
anovas <- map2(comm_ps,names(comm_ps),~{
  ...
  res <- list(
    sitewise=sample_tibble(.x) %>%
      ...
      map(function(comp) {
        comp <<- comp
        comm <- subset_samples(.x,comparison == comp)
        community <- otu_table(comm)
        sd <- sample_tibble(comm)
        dd <- vegdist(community, method = "jaccard")
        tryCatch(
          adonis(dd ~ protection_status, data=sd, permutations = perm),
  ...
  sd <- sample_tibble(.x)
  dd <- vegdist(community, method = "jaccard")
  res$overall <- adonis(dd  ~ protection_status, data=sd, perm=perm)
  res
}) %>% set_names(names(comm_ps))
